// src/app/(main)/university/reports/page.tsx
"use client";
...
export default function ReportsPage() {
    ...
    const loadStats = async () => {
        ...
        try {
            ...
            const res = await authFetch(`/university/reports/${user?.institutionId}/yearly-stats?year=${selectedYear}`);
            if (res.ok) {
                ...
                setStats({
                    ...
                    placementRate: 79.03,
                    branchWise: [
                        { branch: 'Computer Science', totalStudents: 400, placedStudents: 380, averagePackage: 12.5, placementPercentage: 95 },
                        { branch: 'Electrical', totalStudents: 300, placedStudents: 220, averagePackage: 8.2, placementPercentage: 73.3 },
                        { branch: 'Mechanical', totalStudents: 300, placedStudents: 180, averagePackage: 6.4, placementPercentage: 60 },
                        { branch: 'Information Technology', totalStudents: 240, placedStudents: 200, averagePackage: 11.2, placementPercentage: 83.3 },
                    ]
                });
            }
        } catch (err) {
    ...
    const handleDownload = async (type: string, format: 'pdf' | 'excel') => {
        try {
            const res = await authFetch(`/university/reports/${user?.institutionId}/export?year=${selectedYear}&format=${format}&type=${type}`);
            if (res.ok) {
        ...
        <div className="space-y-8 max-w-7xl mx-auto pb-12">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    ...
                    <p className="text-base text-gray-500 dark:text-gray-400 mt-2 font-medium">Generate and export official institutional reports for NIRF, NAAC, and NBA.</p>
                </div>
            ...
            <div className="p-8 rounded-3xl glass border border-gray-200 dark:border-white/5">
                <div className="flex items-center justify-between mb-8">
                    ...
                    <button
                        onClick={() => toast({ title: "Coming Soon", description: "Compliance Checklist will be available shortly." })}
