...
for (let i = 0; i < lines.length; i++) {
    ...
    // Inject the logic
    if (line.includes('// Client-side Analysis') && !hasInjectedLogic) {
        hasInjectedLogic = true;
        newLines.push(`    const fetchAnalytics = useCallback(async () => {
        if (!sessionId) return;
        try {
...
`);
    ...
    // Skip old fetchAnalytics
    if (line.includes('const fetchAnalytics = useCallback(async () => {')) {
        inFetchAnalytics = true;
        continue;
    }
    if (inFetchAnalytics) {
        if (line.includes('}, [sessionId, user]);')) {
            inFetchAnalytics = false;
        }
        continue;
    }

    // Skip submitVideoAnswer
    ...
    if (inHandleStartInterview) {
        if (line.includes('// Handle interview end with fullscreen exit') || line.includes('// Violation Counter')) {
    ...
    if (inControls) {
        ...
        continue; // skip the old controls
    }
...
console.log('Fixed mixed-room correctly');
