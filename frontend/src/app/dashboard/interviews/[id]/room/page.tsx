            ...
            <header className="flex items-center justify-between px-6 py-4 border-b border-white/5">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h1 className="text-lg font-semibold text-white">{session.targetRole} Role</h1>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-red-500/20 border border-red-500/30">
                        ...
                        <span className="text-red-400 text-sm font-medium">REC • {formatElapsedTime(elapsedTime)}</span>
                    </div>

                    {/* Mic toggle */}
                    <button
                        onClick={() => setIsMuted(!isMuted)}
                        className={`p-2 rounded-lg transition ${isMuted ? 'bg-red-500/20 text-red-400' : 'bg-white/5 text-gray-400 hover:text-white'}`}
                    >
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="p-4 rounded-xl bg-[#111820] border border-white/5">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="text-xs text-teal-400 font-medium">{sentiment.label}</span>
                            </div>
                            <p className={`text-2xl font-bold ${sentiment.type === 'positive' || sentiment.type === 'confident' ? 'text-green-400' : sentiment.type === 'negative' ? 'text-red-400' : 'text-teal-400'}`}>
                        ...
                        <div className="p-4 rounded-xl bg-[#111820] border border-white/5">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="text-xs text-teal-400 font-medium">{analytics?.techAccuracy?.label || 'Good'} ({analytics?.techAccuracy?.score || 0}%)</span>
                            </div>
                            ...
                            <div className="flex items-center justify-center">
                                <div className="relative w-20 h-20">
                                    <svg className="w-full h-full -rotate-90">
                                        <circle cx="40" cy="40" r="35" fill="none" stroke="#1a2530" strokeWidth="6" />
                                        <circle cx="40" cy="40" r="35" fill="none" stroke="#14b8a6" strokeWidth="6" strokeDasharray={`${(speechPacing.wpm / 200) * 220} 220`} />
                                    </svg>
                                    <div className="absolute inset-0 flex items-center justify-center">
                                        <div className="text-center">
                                            <p className="text-xl font-bold text-white">{speechPacing.wpm || 135}</p>
                                            <p className="text-[10px] text-gray-500">WPM</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <p className="text-xs text-teal-400 text-center mt-2">Optimal Pace</p>
                        </div>
                    </div>
                </div>

                {/* Right Panel */}
                <div className="w-96 border-l border-white/5 p-6 flex flex-col gap-4 overflow-y-auto">
                    {/* AI Interviewer Card */}
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-[#111820] border border-white/5">
                        <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-teal-500/50">
                            <img src={interviewer.avatar} alt={interviewer.name} className="w-full h-full object-cover" />
                        </div>
                        <div>
                            <p className="text-xs text-teal-400 uppercase">AI Interviewer</p>
                            <p className="text-white font-medium">{interviewer.name} ({interviewer.role})</p>
                        </div>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-[#111820] border border-white/5">
                        <div className="flex items-center justify-between mb-3">
                            <span className="text-xs text-red-400 uppercase tracking-wider">Current Question</span>
                    ...
                    <div className="p-4 rounded-xl bg-gradient-to-br from-amber-500/10 to-orange-500/10 border border-amber-500/20">
                        ...
                        {loadingHint ? (
                            ...
                            <div>
                                <p className="text-gray-300 text-sm">{aiHint.hint}</p>
