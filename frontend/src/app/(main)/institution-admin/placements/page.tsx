// src/app/(main)/institution-admin/placements/page.tsx
"use client";
...
export default function PlacementsPage() {
    ...
    const loadPlacements = async () => {
        setLoading(true);
        try {
            // Fetch for placements
            const res = await authFetch('/institution-admin/placements');
            if (res.ok) {
                ...
                const accepted = fetchedOffers.filter((o: PlacementOffer) => o.status === 'ACCEPTED');
                const avg = accepted.length > 0 ? accepted.reduce((sum: number, o: PlacementOffer) => sum + o.package, 0) / accepted.length : 0;
                const highest = fetchedOffers.length > 0 ? Math.max(...fetchedOffers.map((o: PlacementOffer) => o.package || 0)) : 0;

                // Group by company for chart
                const companyMap: Record<string, number> = {};
                fetchedOffers.forEach((o: PlacementOffer) => {
                    if (o.company) {
                ...
                setStats({
                    totalOffers: fetchedOffers.length,
                ...
                setOffers([]);
                setStats({ totalOffers: 0, highestPackage: 0, averagePackage: 0, topCompanies: [] });
            }
        } catch (err) {
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <div className="flex items-center gap-3 mb-1">
                        ...
                        <h2 className="text-2xl font-black text-gray-900 dark:text-white">Placement Tracking</h2>
                    </div>
                    <p className="text-sm text-gray-500 dark:text-gray-400 ml-11">Monitor job offers, salaries, and recruiting company trends.</p>
                </div>
