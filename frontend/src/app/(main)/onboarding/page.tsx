...
import {
    ...
    ArrowRight, Target, Zap, Trophy, Users, GraduationCap, ArrowLeft
} from 'lucide-react';
...
const ROLE_CATEGORIES = ['Engineering', 'Data', 'Design', 'Product', 'Management', 'Marketing', 'Operations', 'Security'];
...
export default function OnboardingPage() {
    ...
    // Set active category when roles load
    useEffect(() => {
    ...
    const handleJobRoleComplete = async () => {
        ...
        if (success) {
            ...
            setError('Failed to save your job role selection. Please try again.');
        }
    };

    const handleInstitutionComplete = async (skip = false) => {
        setError(null);
        try {
            const res = await authFetch(`/auth/profile`, {
                ...
                body: JSON.stringify({ institutionId: skip ? null : (formData.institutionId || null) })
            });
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-5 py-10 bg-[#F7F9FC] dark:bg-[#060D1E] overflow-y-auto">
                ...
                <div className="w-full max-w-[420px] mb-7">
                    <div className="flex items-start">
                        {[1, 2, 3].map((s) => {
                            const isCompleted = s === 1 || (s === 2 && step > 1);
                            const isActive = (s === 2 && step === 1) || (s === 3 && step === 2);
                            return (
                                <div key={s} className={`flex items-center ${s < 3 ? 'flex-1' : ''}`}>
                                    ...
                                    {s < 3 && (
                                        <div className={`h-px flex-1 mx-3 mb-4 transition-all duration-500 ${
                                            isCompleted ? 'bg-green-500' : 'bg-gray-200 dark:bg-white/10'
                                        }`} />
                                    )}
                                </div>
                            );
                                        ...
                                        visibleRoles.map((role) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`relative p-3 rounded-xl text-left text-sm font-medium transition-all border ${
                                                        selected
                        ...
                        {step === 2 && (
                            ...
                            >
                                ...
                                <div className="space-y-2">
                                    <button
                                        type="button"
                                        onClick={() => handleInstitutionComplete(false)}
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => handleInstitutionComplete(true)}
                                        disabled={isLoading}
