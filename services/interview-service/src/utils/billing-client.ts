...
export class BillingClient {
    ...
    static async consumeCredit(
        ...
        try {
            ...
            // For network errors, we'll allow the request to proceed
            // but log it for monitoring. You can change this behavior
            // to fail-closed if preferred.
     ...
     */
    static async getBalances(authHeader: string): Promise<Record<CreditType, number>> {
        try {
            const response = await fetch(`${BILLING_SERVICE_URL}/credits/balances`, {
                headers: {
                    'Authorization': authHeader,
                },
            });
            ...
            if (!response.ok) {
                return { RESUME_REVIEW: 0, AI_INTERVIEW: 0, SKILL_TEST: 0 };
            }

            return data.data || { RESUME_REVIEW: 0, AI_INTERVIEW: 0, SKILL_TEST: 0 };
        } catch (error: any) {
            logger.error(`Billing service balance error: ${error.message}`);
            return { RESUME_REVIEW: 0, AI_INTERVIEW: 0, SKILL_TEST: 0 };
        }
    }
}
