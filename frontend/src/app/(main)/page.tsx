...
export default function HomePage() {
    ...
    return (
        <div className={`min-h-screen overflow-hidden landing-page ${isLightMode ? 'bg-gray-50' : 'bg-[#0B0F19] bg-grid'}`}>
            <script
            ...
            />
            {/* Navigation */}
            <nav className={`fixed top-0 w-full z-50 ${isLightMode ? 'bg-[rgb(254,255,255)] border-b border-gray-200' : 'glass-premium'}`}>
                    ...
                    <div className="flex items-center justify-between h-16">
                        <div className="flex items-center gap-2">
                            <Image
                                ...
                                width={32}
                                height={32}
                                className={`w-8 h-8 rounded-lg ${isLightMode ? 'shadow-md' : 'neon-purple'}`}
                            />
                            <span className={`text-xl font-bold ${isLightMode ? 'text-indigo-600' : 'gradient-text'}`}>PlaceNxt</span>
                        </div>
                        ...
                        <div className="hidden md:flex items-center gap-4">
                            <Link
                                href="/blog"
                                className={`${isLightMode ? 'text-gray-800 hover:text-indigo-600 font-medium' : 'text-gray-400 hover:text-white'} transition-colors`}
                            >
                                Blog
                            </Link>
                            <Link
                            ...
                            <Link
                                href="/login"
                                className={`${isLightMode ? 'text-gray-800 hover:text-indigo-600 font-medium' : 'text-gray-300 hover:text-white'} transition-colors`}
                            >
                                Login
                            </Link>
                            <Link
                                href="/register"
                                style={{ backgroundColor: 'rgb(124, 113, 243)' }}
                                className="px-4 py-2 rounded-lg text-white font-medium hover:opacity-90 transition-opacity neon-purple"
                            >
                                Get Started
                            </Link>
                            <ThemeToggle />
                        ...
                        <div className="md:hidden flex items-center gap-2">
                            ...
                            <button
                                ...
                                className={`p-2 rounded-lg ${isLightMode ? 'hover:bg-gray-100 text-gray-700' : 'hover:bg-white/10 text-gray-300'} transition-colors`}
                ...
                <AnimatePresence>
                    {mobileMenuOpen && (
                        <motion.div
                            ...
                            className={`md:hidden border-t ${isLightMode ? 'border-gray-200 bg-white/95' : 'border-white/10 bg-gray-900/95'} backdrop-blur-lg`}
                        >
                            <div className="px-4 py-4 space-y-3">
                                <Link
                                    href="/blog"
                                    onClick={() => setMobileMenuOpen(false)}
                                    className={`block px-4 py-3 rounded-lg font-medium ${isLightMode ? 'text-gray-800 hover:text-indigo-600 hover:bg-gray-100' : 'text-gray-300 hover:text-white hover:bg-white/10'} transition-colors`}
                                >
                                    Blog
                                </Link>
                                <Link
                                ...
                                <Link
                                    ...
                                    onClick={() => setMobileMenuOpen(false)}
                                    style={{ backgroundColor: 'rgb(124, 113, 243)' }}
                                    className="block px-4 py-3 rounded-lg text-white font-medium hover:opacity-90 transition-opacity text-center neon-purple"
                                >
                                    Get Started
                                </Link>
                            </div>
            ...
            <section className="pt-24 pb-12 md:pt-32 md:pb-20 px-4 relative">
                {/* Background Glow */}
                <div className={`absolute top-1/4 left-1/2 -translate-x-1/2 w-[800px] h-[600px] ${isLightMode ? 'bg-indigo-500/10' : 'bg-indigo-500/20'} rounded-full blur-[150px] pointer-events-none`} />

                <div className="max-w-7xl mx-auto text-center relative z-10">
                    <motion.div
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${isLightMode ? 'bg-[rgba(255, 255, 255, 1)] shadow-md border border-gray-200' : 'glass-premium'} mb-6`}
                    >
                        <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                        <span className={`text-sm ${isLightMode ? 'text-gray-600' : 'text-gray-300'}`}>AI-Powered Placement Platform</span>
                    </motion.div>
                    ...
                    >
                        ...
                        <br />
                        <span className={isLightMode ? 'text-gray-900' : 'text-white'}>Starts Here</span>
                    </motion.h1>
                    ...
                    <motion.p
                        ...
                        className={`text-base sm:text-lg md:text-xl ${isLightMode ? 'text-gray-600' : 'text-gray-400'} max-w-2xl mx-auto mb-8 md:mb-10`}
                    >
                    ...
                    >
                        ...
                        <Link
                            href="#roadmap"
                            className={`w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 rounded-xl ${isLightMode ? 'bg-[rgb(225,228,230)] border border-gray-200 text-gray-900 shadow-sm hover:border-indigo-300' : 'glass-card text-white hover:border-indigo-500/50'} font-semibold text-base sm:text-lg transition-colors`}
                        >
                            See How It Works
                        </Link>
                    </motion.div>
                    ...
                    <motion.div
                        ...
                        className={`mt-16 flex flex-col sm:flex-row items-center justify-center gap-8 ${isLightMode ? 'text-gray-600' : 'text-gray-400'}`}
                    >
                        <div className="flex items-center gap-2">
                            <Users className="w-5 h-5" />
                            <span><strong className={isLightMode ? 'text-gray-900' : 'text-white'}>10,000+</strong> Students Placed</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <Star className="w-5 h-5 text-yellow-400" />
                            <span><strong className={isLightMode ? 'text-gray-900' : 'text-white'}>4.9/5</strong> User Rating</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <TrendingUp className="w-5 h-5 text-green-400" />
                            <span><strong className={isLightMode ? 'text-gray-900' : 'text-white'}>85%</strong> Success Rate</span>
                        </div>
                    </motion.div>
                </div>
            </section>
            ...
            {/* Footer */}
            <footer className={`py-8 px-4 border-t ${isLightMode ? 'border-gray-200' : 'border-white/5'}`}>
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
                    <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded bg-gradient-to-br from-indigo-500 to-violet-500" />
                        <span className={isLightMode ? 'text-gray-600' : 'text-gray-400'}>© 2026 PlaceNxt. All rights reserved.</span>
                    </div>
                    <div className={`flex items-center gap-6 ${isLightMode ? 'text-gray-600' : 'text-gray-400'}`}>
                        <Link href="/pricing" className={`${isLightMode ? 'hover:text-gray-900' : 'hover:text-white'} transition-colors`}>Pricing</Link>
                        <Link href="/privacy" className={`${isLightMode ? 'hover:text-gray-900' : 'hover:text-white'} transition-colors`}>Privacy</Link>
                        <Link href="/terms" className={`${isLightMode ? 'hover:text-gray-900' : 'hover:text-white'} transition-colors`}>Terms</Link>
                        <Link href="/contact" className={`${isLightMode ? 'hover:text-gray-900' : 'hover:text-white'} transition-colors`}>Contact</Link>
                    </div>
                </div>
            </footer>
        </div>
