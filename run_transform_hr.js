...
for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes('const submitVideoAnswer = async () => {')) {
        ...
        // Inject our handle function here
        newLines.push(`
...
`);
    ...
    if (line.includes('const handleEndInterview = async () => {')) {
        ...
        // skip original
        while (!lines[i + 1].includes('return (')) {
            i++;
        }
        continue;
    }
    ...
    if (line.includes('const handleStartInterview = async () => {')) {
        ...
        newLines.push('        }');
        newLines.push('    };');
        while (!lines[i + 1].includes('// Handle interview end with fullscreen exit')) {
            i++;
        }
        continue;
    }
