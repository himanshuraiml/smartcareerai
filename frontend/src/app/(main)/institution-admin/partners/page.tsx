...
interface Partnership {
    ...
    recruiterContacts: { id: string; name: string; email: string }[];
    isActive: boolean;
}
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            <div className="flex justify-between items-end flex-wrap gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 ml-11">Manage institutional partnerships, MOUs and key accounts.</p>
                </div>
                <div className="flex gap-3">
                    <button
                        onClick={() => setIsAdding(true)}
                        className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-sm font-bold shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/30 transition active:scale-95"
                    >
                        <Plus className="w-4 h-4" />
                        Add New Partner
                    </button>
                    <button className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-white/10 text-gray-700 dark:text-gray-300 px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                    ...
                    >
                        ...
                        <div className="mt-6 pt-4 border-t border-gray-200 dark:border-white/5">
                            ...
                            <div className="flex gap-2">
                                <button
                                    onClick={() => {
                                        const contactId = partner.recruiterContacts[0]?.id || '';
                                        const targetPath = `/institution-admin/messages?contactId=${contactId}`;
                                        if (pathname === '/institution-admin/messages' && searchParams.get('contactId') === contactId) {
                                            router.back();
                                        } else {
                                            router.push(targetPath);
                                        }
                                    }}
                                    className="flex-1 bg-violet-600 py-2 px-3 rounded-xl text-white hover:bg-violet-500 transition flex items-center justify-center gap-2 group/btn active:scale-95"
                                >
                                ...
                                <button className="flex-1 bg-gray-50 dark:bg-white/5 py-2 px-3 rounded-xl border border-gray-200 dark:border-white/10 hover:border-violet-500/50 transition flex items-center justify-center gap-2 group/btn">
                                    ...
                                    <span className="text-xs font-bold text-gray-700 dark:text-gray-300 group-hover/btn:text-violet-500 transition">Email</span>
                                </button>
